...
// Nécessaire en mode "hybrid" avec Vercel où les .env ne sont pas chargées automatiquement dans les API routes
// En production sur Vercel, les variables sont déjà dans process.env
try {
  const root = process.cwd();
  // Charger .env en premier, puis .env.local pour écraser (si les fichiers existent)
  config({ path: resolve(root, ".env") });
  config({ path: resolve(root, ".env.local") });
} catch (error) {
  // Ignorer les erreurs de chargement (les fichiers peuvent ne pas exister en production)
}

const REQUIRED_ENV = ["SMTP_HOST", "SMTP_PORT", "SMTP_USER", "SMTP_PASS", "CONTACT_TO"] as const;

const missingEnv = REQUIRED_ENV.filter((key) => !process.env[key] || process.env[key] === "");

// Debug: afficher les variables chargées au démarrage (si des variables manquent)
...
const formatLine = (label: string, value: string) => `<strong>${label} :</strong> ${escapeHtml(value || "—")}`;
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const attachmentsDisplay = attachmentsCount
      ? attachments
          .map((file) => `• ${escapeHtml(file.filename || "pièce jointe")}`)
          .join("<br/>")
      : "Aucune";

    const subjectFragments: string[] = ["Lead A.R. Euro Forêt"];
    ...
    const textBody = [
      ...
      ``,
      attachmentsCount
        ? `Pièces jointes  : ${attachmentsCount} ${attachmentsCount > 1 ? "fichiers" : "fichier"} (${attachments
    ...
    const replySections = [
      ...
      `Pièces jointes : ${
        attachmentsCount
          ? `${attachmentsCount} ${attachmentsCount > 1 ? "fichiers" : "fichier"} (${attachments
              .map((file) => file.filename)
              .join(", ")})`
          : "aucune"
      }`,
    ...
    const htmlBody = `
      <div style="font-family:system-ui,Arial,sans-serif;max-width:640px;margin:auto;padding:16px;">
        ...
        <p style="margin:12px 0;">
          ${
            telephone
              ? `<a href="tel:${escapeHtml(telephone)}" style="text-decoration:none;padding:10px 14px;border:1px solid #ccc;border-radius:6px;">📞 Appeler</a>`
              : ""
          }
          ${
            email
        ...
        <table style="width:100%;border-collapse:collapse;margin:12px 0;">
          ...
          <tr><td style="padding:8px;border-bottom:1px solid #eee;">Téléphone</td><td style="padding:8px;border-bottom:1px solid #eee;">${
